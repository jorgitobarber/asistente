...
const parseMessageWithGemini = (text) => {
  try {
    ...
    const systemInstruction = `
      ...
      1. El JSON devuelto debe ser estrictamente válido. No omitas comas ni agregues comas al final de las listas.
      2. NUNCA uses comillas dobles (") dentro de los valores de texto. Si necesitas enfatizar o citar, usa SIEMPRE comillas simples (').
      3. No uses saltos de línea ni caracteres especiales no escapados dentro de los strings.
    `;
    ...
    while (attempt < retries) {
      ...
      if (responseCode === 200) {
        ...
        attempt++;
        console.warn(`[GEMINI] Error ${responseCode} (Rate Limit / Sobrecarga). Intento ${attempt} de ${retries}...`);
        if (attempt >= retries) {
          throw new Error(`Error API Gemini tras ${retries} intentos. Código: ${responseCode}, Detalle: ${response.getContentText()}`);
        }
        
        // Si el API pide esperar X segundos (común en 429), le damos 10 segundos para asegurarnos
        let waitTime = (responseCode === 429) ? 10500 : (2000 * attempt);
    ...
    // Buscamos la primera llave y la última para extraer solo el JSON
    ...
    // A veces Gemini inserta comas extra antes de cerrar llaves/corchetes, lo limpiamos rápido
    ...
    try {
      ...
      throw new Error(`Error de parseo JSON. Gemini respondió mal. Detalle: ${parseError.message} | JSON: ${cleanJsonText.substring(0, 50)}...`);
    }
