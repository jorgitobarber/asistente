...
const enviarResumenMatutino = () => {
  try {
    ...
    const prompt = `Hoy es ${hoy.toLocaleDateString()}. Eres el asistente de Jorge (estudiante y barbero).
...
const generarReporteBajoDemanda = (accion, chatId) => {
  try {
    ...
    const prompt = `Jorge pidió un reporte financiero. Periodo analizado: ${dateStart.toLocaleDateString()} al ${dateEnd.toLocaleDateString()}.
